...
<div ref={transportRef} className="mb-10">
  ...
  {trip.transport_options && Object.entries(trip.transport_options).map(([mode, options]) => (
          ...
          <thead>
            <tr className="bg-gray-100">
              <th className="border px-4 py-2 text-center">
                <span role="img" aria-label="departure" className="inline-block mr-1">⏰</span>
                Departure
              </th>
              <th className="border px-4 py-2 text-center">
                <span role="img" aria-label="duration" className="inline-block mr-1">⏳</span>
                Duration
              </th>
              <th className="border px-4 py-2 text-center">
                <span role="img" aria-label="price" className="inline-block mr-1">💵</span>
                Price
              </th>
              <th className="border px-4 py-2 text-center">
                <span role="img" aria-label="date" className="inline-block mr-1">📅</span>
                Date
              </th>
              <th className="border px-4 py-2 text-center">
                <span role="img" aria-label="route" className="inline-block mr-1">📍</span>
                Route
              </th>
            </tr>
          </thead>
          <tbody>
            {options.map((option, index) => (
              <tr key={index} className="even:bg-gray-50">
                <td className="border px-4 py-2 text-center">{option.departure_time || option.departure}</td>
                <td className="border px-4 py-2 text-center">{option.duration}</td>
                <td className="border px-4 py-2 text-center">{option.bus_price || option.flight_price || "N/A"}</td>
                <td className="border px-4 py-2 text-center">{option.train_date || option.flight_date || option.bus_arrival_date}</td>
                <td className="border px-4 py-2 text-center">{option.source} → {option.destination}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {hotels.length ? hotels.map((hotel, index) => (
              <Card key={index} className="border-foreground/20 p-3 hover:scale-105 transition-transform duration-300">
                <img src={hotel.image_url} className="h-48 w-full object-cover rounded-lg" alt={hotel.name} />
                <CardHeader className="text-center">
                  <CardTitle className="text-xl font-semibold text-primary/80">{hotel.name}</CardTitle>
                </CardHeader>
                <CardContent className="text-sm text-gray-700">
                  <p>⭐ <strong>Rating:</strong> {hotel.rating}</p>
                  <p>📍 <strong>Address:</strong> {hotel.vicinity}</p>
